...
def main():
    parser = argparse.ArgumentParser(description="Plot Designability Comparison (Experimental vs Baseline)")
    ...
    # Check columns
    required = ['length_bin', 'Experimental_Designability', 'Baseline_Designability']
    for c in required:
    ...
    df['Baseline_scRMSD_mean'], df['Baseline_scRMSD_std'] = zip(*df['Baseline_scRMSD'].map(parse_scrmsd))
    df['Experimental_scRMSD_mean'], df['Experimental_scRMSD_std'] = zip(*df['Experimental_scRMSD'].map(parse_scrmsd))

    # --- Plot 1: Designability ---
    sns.set(style="whitegrid")
    plt.figure(figsize=(10, 6))
    
    sns.lineplot(data=df, x='length_bin', y='Baseline_Designability', marker='o', label='Baseline', linewidth=2.5)
    sns.lineplot(data=df, x='length_bin', y='Experimental_Designability', marker='o', label='Experimental', linewidth=2.5)
    ...
    # Baseline
    plt.plot(x_nums, df['Baseline_scRMSD_mean'], marker='o', label='Baseline', linewidth=2.5)
    plt.fill_between(x_nums, 
                     df['Baseline_scRMSD_mean'] - df['Baseline_scRMSD_std'], 
                     df['Baseline_scRMSD_mean'] + df['Baseline_scRMSD_std'], 
                     alpha=0.2)
                     
    # Experimental
    plt.plot(x_nums, df['Experimental_scRMSD_mean'], marker='o', label='Experimental', linewidth=2.5)
    plt.fill_between(x_nums, 
                     df['Experimental_scRMSD_mean'] - df['Experimental_scRMSD_std'], 
                     df['Experimental_scRMSD_mean'] + df['Experimental_scRMSD_std'], 
                     alpha=0.2)
